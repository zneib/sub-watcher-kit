...
<div>
  ...
  <button disabled={optionsData.maxActivePlayers - activePlayerData?.length <= 0} class="wrapper" on:click={() => handleClick(person)} on:pointerdown={handlePointerEvent}>
    {person.playerName}
    <div>
      <svg xmlns="http://www.w3.org/2000/svg" on:click|stopPropagation={(e) => showEditDialogElement(person.playerName)} fill="none" width="12px" height="12px" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
